        ...
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-2">
                <a href="{{ route('patients.show', $patient) }}">
                    <x-button>{{ __('View') }}</x-button>
                </a>
                <a href="{{ route('patients.index') }}">
                    <x-button>{{ __('Back') }}</x-button>
                </a>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            @if($errors->any())
                <div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-4" role="alert">
                    <p class="font-bold">{{ __('Error updating patient') }}</p>
                    <ul>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('patients.update', $patient) }}" method="POST" id="form">
                        ...
                        <div class="mb-6">
                            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">{{ __('Patient Data') }}</h3>
                        ...
                        <div class="mb-6">
                            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">{{ __('Responsibles') }} {{__('(2 needed)')}}</h3>
                            ...
                            @foreach($patient->responsibles as $index => $responsible)
                                <div class="p-4 border border-gray-200 dark:border-gray-700 rounded-md mb-4">
                                    <h4 class="font-medium text-gray-900 dark:text-gray-100 mb-3">{{ __('Responsible') }} {{ $index + 1 }}</h4>
                                    
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <input type="hidden" name="responsible[{{ $index }}][id]" value="{{ $responsible->id }}">
                                        
                                        <div>
                                            <label for="responsible[{{ $index }}][name]" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Full Name') }}</label>
                                        ...
                                        <div>
                                            <label for="responsible[{{ $index }}][cpf]" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('CPF') }}</label>
                                        ...
                                        <div>
                                            <label for="responsible[{{ $index }}][relationship]" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Relationship') }}</label>
